...
def compress_old_data() -> dict:
    ...
    if stats_rows:
        ...
        for i in range(0, len(stats_rows), chunk_size):
            ...
            client.table("monthly_stats").upsert(
                chunk, on_conflict="hotel_id,month,room_type"
            ).execute()
        log.info("stats_upserted", count=len(stats_rows))

    # 4. 원본 daily_rates 삭제 (cutoff 이전)
    del_res = client.table("daily_rates") \
        .delete() \
        .lt("stay_date", cutoff.isoformat()) \
        .execute()
    ...
    client.table("price_changes") \
        .delete() \
        .lt("stay_date", cutoff.isoformat()) \
        .execute()

    log.info("compress_completed",
