...
def save_room_rates(hotel_uuid: str, check_in: str, api_response: dict, holidays: set[date]) -> dict:
    ...
    try:
        client.table("room_rates").upsert(
            rows, on_conflict="hotel_id,stay_date,source,room_id,rate_code"
        ).execute()
    except Exception as e:
    ...
    if daily_rows:
        try:
            client.table("daily_rates").upsert(
                daily_rows, on_conflict="hotel_id,stay_date,room_type"
            ).execute()
        except Exception as e:
    ...
    log.info("room_rates_saved", hotel=hotel_uuid[:8], date=check_in,
             room_rates=len(rows), daily_rates=len(daily_rows))
